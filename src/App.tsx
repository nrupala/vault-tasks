        ...
        >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full ${mode === 'delete' ? 'bg-red-600 hover:bg-red-700' : 'bg-primary hover:bg-primary/90'} text-primary-foreground font-medium py-3 rounded-xl transition-all flex justify-center items-center gap-2 mt-6 shadow-lg active:scale-[0.98]`}
            >
              {loading ? (
                <div className="w-5 h-5 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />
              ) : (
                <>
                  {mode === 'create' ? <Plus className="w-5 h-5" /> : mode === 'delete' ? <AlertTriangle className="w-5 h-5 text-red-200" /> : <Lock className="w-5 h-5" />}
...
function MainApp() {
  ...
  return (
    <AppShell activeTab={activeTab as any} onTabChange={(tab) => setActiveTab(tab as any)}>
      <motion.div
        key={activeTab}
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
